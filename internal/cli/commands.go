...
func (c *ScanCmd) Run(cli *CLI) error {
	// Context is available via cli.ctx for cancellation
	// TODO: Implement scan logic in Phase 6
	// When collector is wired up, pass cli.ctx to CollectProject
...
func (c *GenerateCmd) Run(cli *CLI) error {
	...
	// When graph building is implemented, pass cli.ctx for cancellation support
...
func (c *VersionCmd) Run(cli *CLI) error {
	// Context is available via cli.ctx if needed (though version doesn't need it)
